...
$$ \mathbf{x_m} = | \mathbf{x_m}|e^{i\theta_m} \tag{1} $$
...
$$ f(\mathbf{x_m}) \cdot f(\mathbf{x_n}) = g(\mathbf{x_m},\mathbf{x_n}, m - n) \tag{2} $$
...
$$ f(\mathbf{x_m}) = \mathbf{x_m}e^{im\theta} $$
...
Now that we have a candidate for $f$, let's verify that it really does satisfy property $(2)$. Recall the dot product of two vectors represented as complex numbers is:

$$ \mathbf{x_m} \cdot \mathbf{x_n} = \Re[\mathbf{x_m}\mathbf{x_n}] $$

If we apply $f$ to both \mathbf{x_m} and \mathbf{x_n}, then this becomes:

$$ \begin{align} f(\mathbf{x_m}) \cdot f(\mathbf{x_n}) &= \Re[\mathbf{x_m}\mathbf{x_n}e^{i(m - n)\theta}] \\
&= \Re[|\mathbf{x_m}||\mathbf{x_n}|e^{i(\theta_m - \theta_n + [m - n]\theta)}] \\
&=  |\mathbf{x_m}||\mathbf{x_n}| \cos(\theta_m - \theta_n + [m - n]\theta)\end{align} $$
