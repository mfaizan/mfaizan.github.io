...
$$ f(\mathbf{x_m}) \cdot f(\mathbf{x_n}) = g(\mathbf{x_m},\mathbf{x_n}, m - n) \tag{1} $$
...
Now that we have a candidate for $f$, let's verify that it really does satisfy property $(1)$. Recall the dot product of two vectors represented as complex numbers is:

$$ \mathbf{x_m} \cdot \mathbf{x_n} = \Re[\mathbf{x_m}\mathbf{x^\star_n}] $$

If we apply $f$ to both $\mathbf{x_m}$ and $\mathbf{x_n}$, then this becomes:

$$ \begin{align} f(\mathbf{x_m}) \cdot f(\mathbf{x_n}) &= \Re[\mathbf{x_m}\mathbf{x^\star_n}e^{i(m - n)\theta}] \\
